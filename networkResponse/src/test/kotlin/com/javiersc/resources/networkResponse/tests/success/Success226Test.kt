...
internal class Success226Test : BaseTest<Dog> {
    ...
    override val codeToFile: Pair<Int, String?> get() = StatusCode.IM_USED_226.code to "2xx.json"
    override val expected = Dog(1, "Auri", 7)

    @Test
    fun `suspend call`() = runBlocking {
        with(service.getDog() as ImUsed) {
    ...
    fun `async call`() = runBlocking {
        with(service.getDogAsync().await() as ImUsed) {
            value shouldBe expected
            headers.toHeaderList() shouldContain expectedHeader
        }
    }
