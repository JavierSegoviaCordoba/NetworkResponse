...
internal class NotAcceptable406Test : BaseTest<Error> {
    ...
    fun `suspend call`() = runBlocking {
        ...
        val error: Error? = (response as NotAcceptable).error
    ...
    fun `mapping concrete NetworkResponse to Resource`() = runBlocking {
        ...
        val resource: Resource<String, String> =
            response.toResource(Dog::name, Error?::unused, mapNotAcceptable = Error?::text)
        val name: String? = (resource as Resource.Error).error
        name shouldBe expected.message
    }
}

internal class NullNotAcceptable406Test : BaseNullTest<Error>(406) {
