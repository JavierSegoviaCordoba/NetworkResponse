...
internal class Success202Test : BaseTest<Dog> {
    ...
    override val codeToFile: Pair<Int, String?> get() = StatusCode.ACCEPTED_202.code to "2xx.json"
    override val expected = Dog(1, "Auri", 7)

    @Test
    fun `suspend call`() = runBlocking {
        with(service.getDog() as Accepted) {
    ...
    fun `async call`() = runBlocking {
        with(service.getDogAsync().await() as Accepted) {
            value shouldBe expected
            headers!!.toHeaderList() shouldContain expectedHeader
        }
    }
