...
internal class Error451Test : BaseTest<Error> {

    override val codeToFile: Pair<Int, String?> =
        StatusCode.UNAVAILABLE_FOR_LEGAL_REASONS_451.code to "4xx.json"
    override val expected: Error = Error("Dog has some error")

    @Test
    fun `suspend call`() = runBlocking {
        with(service.getDog() as UnavailableForLegalReasons) {
    ...
    fun `async call`() = runBlocking {
        with(service.getDogAsync().await() as UnavailableForLegalReasons) {
            error shouldBe expected
            headers.toHeaderList() shouldContain expectedHeader
        }
    }
    ...
    fun `mapping concrete NetworkResponse to Resource`() = runBlocking {
        val resource: Resource<String, String> = service.getDog()
            .toResource(Dog::unused, Error?::unused, unavailableForLegalReasons = Error?::text)
        (resource as Resource.Error).error shouldBe expected.message
    }
}

internal class ErrorNull451Test : BaseNullTest<Error?>(
    StatusCode.UNAVAILABLE_FOR_LEGAL_REASONS_451.code
) {

    @Test
    fun `suspend call with null error`() = runBlocking {
        (service.getDog() as UnavailableForLegalReasons).error shouldBe expected
    }
}
