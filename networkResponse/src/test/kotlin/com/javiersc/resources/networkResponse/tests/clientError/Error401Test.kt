...
internal class Error401Test : BaseTest<Error> {

    override val codeToFile: Pair<Int, String?> = StatusCode.UNAUTHORIZED_401.code to "4xx.json"
    override val expected: Error = Error("Dog has some error")

    @Test
    fun `suspend call`() = runBlocking {
        with(service.getDog() as Unauthorized) {
    ...
    fun `async call`() = runBlocking {
        with(service.getDogAsync().await() as Unauthorized) {
            error shouldBe expected
            headers.toHeaderList() shouldContain expectedHeader
        }
    }
    ...
    fun `mapping concrete NetworkResponse to Resource`() = runBlocking {
        val resource: Resource<String, String> =
            service.getDog().toResource(Dog::unused, Error?::unused, unauthorized = Error?::text)
        (resource as Resource.Error).error shouldBe expected.message
    }
}

internal class ErrorNull401Test : BaseNullTest<Error?>(StatusCode.UNAUTHORIZED_401.code) {

    @Test
    fun `suspend call with null error`() = runBlocking {
        (service.getDog() as Unauthorized).error shouldBe expected
    }
}
