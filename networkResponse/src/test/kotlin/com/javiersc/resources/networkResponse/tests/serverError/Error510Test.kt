...
internal class Error510Test : BaseTest<Error> {

    override val codeToFile: Pair<Int, String?> = StatusCode.NOT_EXTENDED_510.code to "5xx.json"
    ...
    fun `mapping concrete NetworkResponse to Resource`() = runBlocking {
        val resource: Resource<String, String> = service.getDog()
            .toResource(Dog::unused, Error?::unused, notExtended = Error?::text)
        (resource as Resource.Error).error shouldBe expected.message
    }
}

internal class ErrorNull510Test : BaseNullTest<Error?>(
    StatusCode.NOT_EXTENDED_510.code
) {

    @Test
    fun `suspend call with null error`() = runBlocking {
        (service.getDog() as NotExtended).error shouldBe expected
    }
}
