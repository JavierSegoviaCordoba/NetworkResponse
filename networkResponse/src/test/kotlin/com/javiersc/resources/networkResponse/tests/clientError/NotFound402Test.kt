...
internal class NotFound402Test : BaseTest<Error> {
    ...
    fun `suspend call`() = runBlocking {
        ...
        val error: Error? = (response as PaymentRequired).error
    ...
    fun `mapping concrete NetworkResponse to Resource`() = runBlocking {
        ...
        val resource: Resource<String, String> =
            response.toResource(Dog::name, Error?::unused, mapPaymentRequired = Error?::text)
        val name: String? = (resource as Resource.Error).error
        name shouldBe expected.message
    }
}

internal class NullNotFound402Test : BaseNullTest<Error>(402) {
