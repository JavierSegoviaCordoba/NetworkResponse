...
internal class NotFound403Test : BaseTest<Error> {
    ...
    fun `suspend call`() = runBlocking {
        ...
        val error: Error? = (response as Forbidden).error
    ...
    fun `mapping concrete NetworkResponse to Resource`() = runBlocking {
        ...
        val resource: Resource<String, String> =
            response.toResource(Dog::name, Error?::unused, mapForbidden = Error?::text)
        val name: String? = (resource as Resource.Error).error
        name shouldBe expected.message
    }
}

internal class NullNotFound403Test : BaseNullTest<Error>(403) {
