...
internal class Error504Test : BaseTest<Error> {

    override val codeToFile: Pair<Int, String?> =
        StatusCode.GATEWAY_TIMEOUT_504.code to "5xx.json"
    override val expected: Error = Error("Dog has some error")

    @Test
    fun `suspend call`() = runBlocking {
        with(service.getDog() as GatewayTimeout) {
    ...
    fun `async call`() = runBlocking {
        with(service.getDogAsync().await() as GatewayTimeout) {
            error shouldBe expected
            headers.toHeaderList() shouldContain expectedHeader
        }
    }
    ...
    fun `mapping concrete NetworkResponse to Resource`() = runBlocking {
        val resource: Resource<String, String> = service.getDog()
            .toResource(Dog::unused, Error?::unused, gatewayTimeout = Error?::text)
        (resource as Resource.Error).error shouldBe expected.message
    }
}

internal class ErrorNull504Test : BaseNullTest<Error?>(StatusCode.GATEWAY_TIMEOUT_504.code) {

    @Test
    fun `suspend call with null error`() = runBlocking {
        (service.getDog() as GatewayTimeout).error shouldBe expected
    }
}
