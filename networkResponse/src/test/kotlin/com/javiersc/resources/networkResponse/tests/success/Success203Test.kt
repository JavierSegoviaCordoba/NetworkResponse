...
internal class Success203Test : BaseTest<Dog> {
    ...
    override val codeToFile: Pair<Int, String?>
        get() = StatusCode.NON_AUTHORITATIVE_INFORMATION_203.code to "2xx.json"
    override val expected = Dog(1, "Auri", 7)

    @Test
    fun `suspend call`() = runBlocking {
        with(service.getDog() as NonAuthoritativeInformation) {
    ...
    fun `async call`() = runBlocking {
        with(service.getDogAsync().await() as NonAuthoritativeInformation) {
            value shouldBe expected
            headers!!.toHeaderList() shouldContain expectedHeader
        }
    }
