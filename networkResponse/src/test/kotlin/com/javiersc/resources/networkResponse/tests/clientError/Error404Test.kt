...
internal class Error404Test : BaseTest<Error> {

    override val codeToFile: Pair<Int, String?> = StatusCode.NOT_FOUND_404.code to "4xx.json"
    override val expected: Error = Error("Dog has some error")

    @Test
    fun `suspend call`() = runBlocking {
        with(service.getDog() as NotFound) {
    ...
    fun `async call`() = runBlocking {
        with(service.getDogAsync().await() as NotFound) {
            error shouldBe expected
            headers!!.toHeaderList() shouldContain expectedHeader
        }
    }
    ...
    fun `mapping concrete NetworkResponse to Resource`() = runBlocking {
        val resource: Resource<String, String> =
            service.getDog().toResource(Dog::name, Error?::unused, mapNotFound = Error?::text)
        (resource as Resource.Error).error shouldBe expected.message
    }
}

internal class NullNotFound404Test : BaseNullTest<Error?>(StatusCode.NOT_FOUND_404.code) {

    @Test
    fun `suspend call with null error`() = runBlocking {
        (service.getDog() as NotFound).error shouldBe expected
    }
}
