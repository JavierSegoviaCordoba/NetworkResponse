...
internal class Error411Test : BaseTest<Error> {

    override val codeToFile: Pair<Int, String?> = StatusCode.LENGTH_REQUIRED_411.code to "4xx.json"
    override val expected: Error = Error("Dog has some error")

    @Test
    fun `suspend call`() = runBlocking {
        with(service.getDog() as LengthRequired) {
    ...
    fun `async call`() = runBlocking {
        with(service.getDogAsync().await() as LengthRequired) {
            error shouldBe expected
            headers!!.toHeaderList() shouldContain expectedHeader
        }
    }
...
internal class ErrorNull411Test : BaseNullTest<Error?>(StatusCode.LENGTH_REQUIRED_411.code) {
    ...
    fun `suspend call with null error`() = runBlocking {
        (service.getDog() as LengthRequired).error shouldBe expected
    }
}
