...
internal class NotFound401Test : BaseTest<Error> {
    ...
    fun `suspend call`() = runBlocking {
        ...
        val error: Error? = (response as Unauthorized).error
    ...
    fun `mapping concrete NetworkResponse to Resource`() = runBlocking {
        ...
        val resource: Resource<String, String> =
            response.toResource(Dog::name, Error?::unused, mapUnauthorized = Error?::text)
        val name: String? = (resource as Resource.Error).error
        name shouldBe expected.message
    }
}

internal class NullNotFound401Test : BaseNullTest<Error>(401) {
