...
internal fun <R, E> handle5xx(
    ...
    when (code.statusCode) {
        ...
        NOT_IMPLEMENTED_501 -> complete(NotImplemented(errorBody, headers))
        BAD_GATEWAY_502 -> complete(BadGateway(errorBody, headers))
        SERVICE_UNAVAILABLE_503 -> complete(ServiceUnavailable(errorBody, headers))
        GATEWAY_TIMEOUT_504 -> complete(GatewayTimeout(errorBody, headers))
        HTTP_VERSION_NOT_SUPPORTED_505 -> complete(HttpVersionNotSupported(errorBody, headers))
        VARIANT_ALSO_NEGOTIATES_506 -> complete(VariantAlsoNegotiates(errorBody, headers))
        INSUFFICIENT_STORAGE_507 -> complete(InsufficientStorage(errorBody, headers))
        LOOP_DETECTED_508 -> complete(LoopDetected(errorBody, headers))
        NOT_EXTENDED_510 -> complete(NotExtended(errorBody, headers))
